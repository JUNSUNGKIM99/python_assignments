             ...
             if opcode == 'ld':
                 try:
                    ...
                    if reg_e > len(self.reg) - 1 : 
                        print(f"SimpleCPU_REGIndexError | IP:{self.ip} | {inst_e} | {reg_e} | 0, {len(self.reg) -1 }")
                        exit(1)
                   ...
                   try:
                       self.mem[self.inst_mem[self.ip][MEM_MEM]] = self.reg[self.inst_mem[self.ip][MEM_SRC_REG]]
                       self.ip += 1
                   except: 
                       reg_e = self.inst_mem[self.ip][MEM_DST_REG]  
                       mem_e = self.inst_mem[self.ip][MEM_MEM]
